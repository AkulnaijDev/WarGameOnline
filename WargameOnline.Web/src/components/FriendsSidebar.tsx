    ...
    <div className="fixed bottom-4 right-4 w-72 bg-slate-800 border border-slate-600 rounded-xl shadow-xl text-sm z-50">
      ...
      >
        ...
        <button
          ...
          className="text-white text-lg absolute right-3 top-1 hover:text-slate-300"
          title={isMinimized ? t('showFriends') : t('hideFriends')}
        >
          {isMinimized ? '➕' : '➖'}
        </button>
        {pendingCount > 0 && (
          <span className="absolute left-3 bg-red-600 text-white text-xs px-2 py-0.5 rounded-full">
            {pendingCount}
          </span>
        )}
      </div>
      ...
      {!isMinimized && (
        <>
            ...
            {friends.map((friend) => (
              <li key={friend.id} className="flex justify-between items-center gap-2 mb-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <button
                    onClick={() => {
                      if (!friend.username || friend.username.startsWith('User#')) return
                      openChat({
                        id: friend.id,
          ...
          {feedback && (
            <p className={`mt-1 mx-1 px-2 py-1 text-xs rounded ${feedback.startsWith('✅') ? 'bg-green-800 text-green-300' : 'bg-red-800 text-red-300'}`}>
              {feedback}
            </p>
          )}
        </>
